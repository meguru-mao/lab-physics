...
## 目标与特色
- 覆盖常见物理实验模块（力学、热学、光学/光电、声学等），配套说明文档在仓库根目录的各实验 Markdown 文件中。
...
## 常见问题
- node_modules、dist、build、unpackage 等已通过 `.gitignore` 忽略；如曾被提交到历史，请使用 `git filter-repo` 或 `BFG` 清理历史后强推远端。
...
## 许可
如未在仓库另行声明许可证，默认遵循内部使用限制；若需开源发布，请补充 LICENSE 文件并标注版权与许可证信息。
